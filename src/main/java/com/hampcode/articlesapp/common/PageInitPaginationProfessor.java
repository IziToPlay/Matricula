...
@Component
public class PageInitPaginationProfessor {
	@Autowired
	...
	public  ModelAndView initPagination(Optional<Integer> pageSize, Optional<Integer> page, String url) {
		...
		Page<Professor> professorsList = professorService.findAll(PageRequest.of(evalPage, evalPageSize));
		PagerModel pager = new PagerModel(professorsList.getTotalPages(), professorsList.getNumber(), BUTTONS_TO_SHOW);
		
		initModelView.addObject("professorsList", professorsList);
	...
	public  ModelAndView initPaginationSearch(Optional<Integer> pageSize, Optional<Integer> page, 
		...
		Page<Professor> professorsList = professorService.findById(id, PageRequest.of(evalPage,evalPageSize));
				/*
