...
@Component
public class PageInitPaginationStudent {
	...
	public  ModelAndView initPagination(Optional<Integer> pageSize, Optional<Integer> page, String url) {
		...
		Page<Student> studentsList = studentService.findAll(PageRequest.of(evalPage, evalPageSize));
		PagerModel pager = new PagerModel(studentsList.getTotalPages(), studentsList.getNumber(), BUTTONS_TO_SHOW);
		
		initModelView.addObject("studentsList", studentsList);
	...
	public  ModelAndView initPaginationSearch(Optional<Integer> pageSize, Optional<Integer> page, 
		...
		Page<Student> studentsList = studentService.findById(id, PageRequest.of(evalPage,evalPageSize));
				/*
